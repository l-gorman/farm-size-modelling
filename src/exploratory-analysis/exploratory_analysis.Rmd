...
plot_summary_subnational_area <- indicator_data %>% 
  group_by(geo_id,iso_country_code) %>% 
  summarise(
    n=n(),
    mean= mean(land_cultivated_ha, na.rm=T),
    sd= sd(land_cultivated_ha, na.rm=T),
    iqr = IQR(land_cultivated_ha, na.rm=T)

  )

big_plot_summary_subnational_area <- plot_summary_subnational_area[plot_summary_subnational_area$n>100,]


ggplot()+
 geom_boxplot(data = plot_summary_subnational_area, mapping = aes(y=n))




ggplot(indicator_data, aes(y=land_cultivated_ha, x=geo_id, color=iso_country_code))+
  geom_boxplot()+
  ylim(0,100)


 aov(land_cultivated_ha ~ iso_country_code,
  data = indicator_data
)

ggplot(plot_summary_subnational_area, aes(x=mean))+
  geom_histogram()+
